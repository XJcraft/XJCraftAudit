...
//			event.setUseInteractedBlock(Event.Result.DENY);
        ...
        if (depBlock != null && depBlock.getType() == Material.COMMAND_BLOCK
            ...
            if (Lang.isEmpty(cmdState.getCommand())) {
                ...
                Location baseLocation =
                        helper.getPlayerLocation(player, "base-location");
                Location cmdLocation =
                        helper.getPlayerLocation(player, "base-cmdlocation");
                ...
                if (baseLocation == null || cmdLocation == null) {
                    player.sendMessage(helper.getPlugin()
                            .getMessage("message.baseNotExit"));
                    return;
                }
                ...
                player.sendMessage(
                        ChatColor.AQUA + helper.getPlugin()
                                .getMessage("message.passageOccupied"));
            }
        }
    }
        ...
        if (status == Status.UNAPPLIED) {
            if (event.isArmorAccept()) {
                ...
                if (Util.isFull(player.getInventory())) {
                    ...
                    player.sendMessage(
                            helper.getPlugin().getMessage("message.tip"));
                } else {
                    player.getWorld().playSound(player.getLocation(),
                            Sound.ENTITY_PLAYER_LEVELUP, .5f, .8f);
                    // give command block
                    giveCmdBlock(player);
                    player.sendMessage(helper.getPlugin()
                    ...
                    helper.setPlayerLocation(player, "apply-location",
                            player.getLocation());
                    helper.applied(player);
                }
            } else {
                player.sendMessage(
                        helper.getPlugin().getMessage("message.applyFail"));
            }
        } else {
            player.sendMessage(
                    helper.getPlugin().getMessage("message.applyDup"));
        }
    ...
    public ItemStack giveSignBlock(Player player) {
        ...
        sItemMeta.setLore(
                helper.getPlugin().getConfig().getStringList("signBlock.lore"));
