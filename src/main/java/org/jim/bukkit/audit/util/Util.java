...
public class Util {
	...
	/*public static void setNiceName(final Player p) {
		if (p == null || !p.isOnline() || !AuditPlugin.getPlugin().getConfig().getBoolean("nickname-prefix.enable", true))
			return;
		...
		executor.submit(new Runnable() {
			...
			public void run() {
				try {
					...
					AuditPlugin.getPlugin().getLogger()
							.log(Level.WARNING, "Set nickname for "+p.getName()+" error", e);
				}
			}
		});
	...
	/*
	public static void testMerge(final File dir){
		executor.submit(new Runnable() {
			
			@Override
			public void run() {
				SamePlayerMerge s = new SamePlayerMerge();
				try {
					s.testMerge(dir);
				} catch (Exception e) {
					e.printStackTrace();
