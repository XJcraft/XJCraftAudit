...
public class Title {
	...
	private static void loadClasses() {
		...
		packetActions = getNMSClass(new String[] { "EnumTitleAction",
				"PacketPlayOutTitle$EnumTitleAction" });
		chatBaseComponent = getNMSClass("IChatBaseComponent");
		nmsChatSerializer = getNMSClass(new String[] { "ChatSerializer",
	...
	public void send(Player player) {
		if (packetTitle != null) {
			...
			try {
				...
				Object connection =
						getField(handle.getClass(), "playerConnection")
								.get(handle);
	...
	public void clearTitle(Player player) {
		try {
			...
			Object handle = getHandle(player);
			Object connection =
					getField(handle.getClass(), "playerConnection").get(handle);
			Object[] actions = packetActions.getEnumConstants();
			Method sendPacket = getMethod(connection.getClass(), "sendPacket");
			Object packet =
					packetTitle.getConstructor(packetActions, chatBaseComponent)
	...
	private static Class<?> getNMSClass(String[] classNames) {
		...
		for (String className : classNames) {
			String fullName =
					"net.minecraft.server." + getVersion() + className;
			try {
				clazz = Class.forName(fullName);
				break;
			} catch (Exception e) {
	...
	private Method getMethod(Class<?> clazz, String name, Class<?>... args) {
		for (Method m : clazz.getMethods())
			if (m.getName().equals(name) && (args.length == 0
